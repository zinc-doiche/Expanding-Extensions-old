...
class StatusGUI(val uuid: String): SquareGUI() {
    ...
    private fun strengthLore(strength: Int, player: Player) = list(empty(),
        text("$strength")
            .color(NamedTextColor.LIGHT_PURPLE)
            .decoration(TextDecoration.BOLD, true),
        empty(),
        text("현재 기본 공격력: ${player.damage.format("#.##")}", NamedTextColor.GRAY)
    )

    private fun swiftnessLore(swiftness: Int, player: Player) = list(empty(),
        text("$swiftness")
            .color(NamedTextColor.LIGHT_PURPLE)
            .decoration(TextDecoration.BOLD, true),
        empty(),
        text("현재 이동속도: ${player.speed.format("#.##")}", NamedTextColor.GRAY)
    )

    private fun balanceLore(balance: Int, player: Player) = list(empty(),
        text("$balance")
            .color(NamedTextColor.LIGHT_PURPLE)
            .decoration(TextDecoration.BOLD, true),
        empty(),
        text("현재 체력: ${player.health.format("#.##")}", NamedTextColor.GRAY)
    )

    private fun concentrationLore(concentration: Int, user: User) = list(empty(),
        text("$concentration")
            .color(NamedTextColor.LIGHT_PURPLE)
            .decoration(TextDecoration.BOLD, true),
        empty(),
        text("현재 치명타 확률: ${user.criticalChance.format("#.##")}", NamedTextColor.GRAY)
    )

    private fun remainLore(status: Status) = list(empty(),
        text("${status.remains}")
            .color(NamedTextColor.WHITE)
            .decoration(TextDecoration.BOLD, true),
        empty(),
        text("총 스탯(잔여 포함): ${status.total} / 600", NamedTextColor.WHITE)
    )
    ...
    override fun open() {
        HeartbeatScope().async {
            ...
            with(user.status) {
                ...
                setItem(item(Material.BOOK, text("남은 스탯 포인트"), remainLore(this)), 4, 0)
            }
...
private fun MutableList<Component>.addText() {
    add(empty())
    add(text("클릭하여 능력치를 올릴 수 있습니다.", NamedTextColor.GRAY))
}